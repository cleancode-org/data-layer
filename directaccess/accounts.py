...
def connect(env="DATABASE_URL", connections=2):
    ...
    return ThreadedConnectionPool(minconns, maxconns, **db_settings)
...
def deposit(conn, user, pin, account, amount):
    ...
    # Fetch the current balance in the account and log it
    record = "deposit ${:0.2f} from account {} | current balance: ${:0.2f}"
    log.info(record.format(amount, account, balance(conn, account)))

@transact
def shop(conn, user, pin, account, amount):
    # Step 1: get balance
    cur_balance= balance(conn, account)
  
    if cur_balance >100:
        # Step 2: update the account value by deducting the amount
        update_balance(conn, account,amount*-1 )
    ...
    # Fetch the current balance in the account and log it
    record = "made purchase ${:0.2f} from account {} | current balance: ${:0.2f}"
    log.info(record.format(amount, account, balance(conn, account)))

@transact
def loan(conn, user, pin, account, amount):
    ...
    # Fetch the current balance in the account and log it
    record = "give loan ${:0.2f} from account {} | current balance: ${:0.2f}"
    log.info(record.format(amount, account, balance(conn, account)))
...
if __name__ == '__main__':
    ...
    def op1():
        ...
        #withdraw('alice', 1234, 1, 299.0)
        shop('alice', 1234, 1, 500.0)

    def op2():
        time.sleep(random.random())
        #deposit('alice', 1234, 1, 75.0)
        #withdraw('alice', 1234, 1, 25.0)
        loan('alice', 1234, 1, 299.0)
